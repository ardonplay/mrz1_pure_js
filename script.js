...
function toDopCode(num) {
    var result = new Array(BIT_DEPTH + 1);
    for (var i = 0; i < num.length; i++) {
        var ch = +!num[i];
        result[i] = ch;
    }
    result = binarySum(result, decToBin(1));
    return result;
}
...
function toIntArr(arr) {
    for (var i = 0; i < arr.length; i++) {
        if (arr[i].match(/^\d+$/)) {
            arr[i] = + arr[i];
            if (arr[i] > 15) {
                alert("incorrect input data, max number is 15: " + arr[i]);
                throw new SyntaxError("incorrect input data, max number is 15: " + arr[i]);
            }
        } else {
...
function start() {
    ...
    for (var i = 0; i < firstVector.length; i++) {
        ...
        firstVector[i].shift(); //shifts to left
    ...
    for (var cicle = 0; cicle < BIT_DEPTH; cicle++) {
        ...
        for (var i = 0; i < numOfPair; i++) {
            if (signs[i] == 0) {
                registersP[i] = binarySum(registersP[i],
                    toDopCode(registersB[i]));
            } else {
                registersP[i] = binarySum(registersP[i],
                    registersB[i]);
            }
